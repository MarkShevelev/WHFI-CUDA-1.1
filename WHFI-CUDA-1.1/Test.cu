...
int main() {
	...
	try {
		...
		UniformGrid<float, 2u, 1u> perp_dfc_field(v_space, v_field_size);
		...
		DeviceMemory dev_memory(field_size * 8 * sizeof(float));
		float *x_next = (float *)dev_memory;
		float *x_prev = x_next + field_size;
		float *along_dfc = x_prev + field_size;
		...
		device::to_device(perp_dfc, along_dfc_field.table);

		diffusion::OneDimensionalMultithreadDiffusion<32u,256u,float> diffusion_solver(1.f, row_size, row_count, x_next, x_prev, along_dfc, a, b, c, d);
...
void dfc_field_init(iki::grid::UniformGrid<float, 2u, 1u> &dfc_field) {
