...
namespace iki { namespace diffusion{ namespace device {
	...
	__global__ void  forward_step_matrix_calculation_kernel(
		...
		a(row_idx, elm_idx) = -0.5 * along_r * along_dfc(row_idx, elm_idx - 1) + 0.5 * 0.25 * mixed_r * (along_mixed_dfc(row_idx + 1, elm_idx) - along_mixed_dfc(row_idx - 1, elm_idx));
		b(row_idx, elm_idx) = T(1) + 0.5 * along_r * (along_dfc(row_idx, elm_idx-1) + along_dfc(row_idx, elm_idx));
		c(row_idx, elm_idx) = -0.5 * along_r * along_dfc(row_idx, elm_idx) - 0.5 * 0.25 * mixed_r * (along_mixed_dfc(row_idx + 1, elm_idx) - along_mixed_dfc(row_idx - 1, elm_idx));
