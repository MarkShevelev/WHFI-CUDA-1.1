...
namespace iki { namespace whfi { namespace device {
	...
	__global__ void first_moment_kernel(table::device::DeviceTable<T> const vdf_table, T const begin, T const step, table::device::DeviceDataLine<T> zero_moment) {
		...
		for (unsigned elm_idx = 2; elm_idx != vdf_table.row_size; elm_idx += 2) {
			auto arg_m = begin + step * (elm_idx - 1), arg = begin + step * elm_idx, arg_p = begin + step * (elm_idx + 1);
			sum += step / T(3.) * (arg_m * vdf_table(row_idx, elm_idx - 1) + T(4.) * arg * vdf_table(row_idx, elm_idx) + arg_p * vdf_table(row_idx, elm_idx + 1));
		}
