...
namespace iki { namespace whfi { namespace device {
	template <typename T>
	__global__ void first_moment_kernel(table::device::DeviceTable<T> const vdf_table, T const begin, T const step, table::device::DeviceDataLine<T> first_moment) {
