...
template <typename T>
std::ostream &operator<<(std::ostream &ascii_os, iki::grid::test::Argument<T> const &arg) {
	return ascii_os << arg[0] << ' ' << arg[1];
}

template <typename T>
std::ostream &operator<<(std::ostream &ascii_os, iki::grid::test::HostGrid<T> const &grid) {
