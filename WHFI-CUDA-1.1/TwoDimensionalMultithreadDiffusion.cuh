...
namespace iki {	namespace diffusion {
	...
	public:
		TwoDimensionalMultithreadDiffusion(T r, unsigned row_size, unsigned row_count, T *x_next, T *x_prev, T *dfc, T *a, T *b, T *c, T *d) : r(r), row_size(row_size), row_count(row_count), x_next(x_next), x_prev(x_prev), dfc(dfc), a(a), b(b), c(c), d(d) { }
